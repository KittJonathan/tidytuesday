...
# Get information about suborders and infraorders ----
...
mygalomorphae <- "https://en.wikipedia.org/wiki/Mygalomorphae" %>% 
  ...
  nth(2) %>% 
  mutate(Genera = as.numeric(Genera)) %>% 
  filter(!is.na(Genera)) %>% 
  mutate(order = "Araneae",
         suborder = "Opisthothelae",
         infraorder = "Mygalomorphae") %>% 
  select(order:infraorder, family = Family)

araneomorphae <- "https://en.wikipedia.org/wiki/Araneomorphae" %>% 
  rvest::read_html() %>% 
  rvest::html_nodes("table.wikitable") %>% 
  rvest::html_table(header = TRUE, na.strings = c(NA, ""), convert = TRUE) %>% 
  nth(2) %>% 
  mutate(Family = gsub("\\s*\\([^\\)]+\\)","", as.character(Family))) %>% 
  mutate(Family = gsub("\\s*\\[[^\\)]+\\]","", as.character(Family))) %>% 
  mutate(order = "Araneae",
         suborder = "Opisthothelae",
...
classification <- rbind(mesothelae, araneomorphae, mygalomorphae)
rm(mesothelae, araneomorphae, mygalomorphae)

# 129 families in spiders vs. 126 families in classification ... ?
diff <- spiders %>% 
  distinct(family) %>% 
  filter(!family %in% classification$family) %>% 
  mutate(order = "Araneae",
         suborder = "Opisthothelae",
         infraorder = "Araneomorphae") %>% 
  select(order:infraorder, family)

classification <- rbind(classification, diff)
...
spiders <- spiders %>% 
  mutate(suborder = case_when(family == "Liphistiidae" ~ "Mesothelae",
                              TRUE ~ "Opisthothelae")) %>% 
  mutate(infraorder = case_when(suborder == "Opisthothelae" & 
                                  family %in% c("Antrodiaetidae", "Atypidae",
